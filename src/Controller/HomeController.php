<?php

namespace App\Controller;

use App\Repository\GroupToursRepository;
use App\Repository\PrivateToursRepository;
use App\Repository\ToursRepository;
use Symfony\Bundle\FrameworkBundle\Controller\AbstractController;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Routing\Annotation\Route;

class HomeController extends AbstractController
{
    #[Route('/', name: 'app_home')]
    public function index(ToursRepository $toursRepository, PrivateToursRepository $privateToursRepository, GroupToursRepository $groupToursRepository): Response
    {
        return $this->render('home/index.html.twig', [
            'controller_name' => 'HomeController',
            'tours' => $toursRepository->findAll(),
            'privateTours' => $privateToursRepository->findAll(),
            'groupTours' => $groupToursRepository->findAll()

        ]);
    }
}
